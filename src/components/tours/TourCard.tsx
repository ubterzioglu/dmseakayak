...
import { Clock, Waves, TrendingUp, MapPin, Check, Star } from "lucide-react";
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={closeModal} labelledBy={titleId}>
        <div className="hero-gradient relative aspect-[16/10] w-full overflow-hidden rounded-t-3xl">
          <img
            src={tour.heroImage}
            alt={pick(tour.title)}
            className="h-full w-full object-cover"
            onError={(e) => (e.currentTarget.style.display = "none")}
          />
          <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-teal-deep">
            {t(`level.${tour.level}`)}
          </span>
        </div>
        ...
        <div className="space-y-5 p-6">
          <div>
            <h3 id={titleId} className="text-2xl font-extrabold text-teal-deep">{pick(tour.title)}</h3>
            <p className="mt-1 text-sm text-ink/75">{pick(tour.tagline)}</p>
          </div>
          ...
          <div className="space-y-4 border-t border-teal/10 pt-5 text-sm">
            <div>
          ...
          <div className="flex items-center justify-between border-t border-teal/10 pt-5">
            <div className="text-lg font-extrabold text-orange">
              {t("common.from")} €{tour.priceEur}
              <span className="ml-1 text-xs font-medium text-ink/60">/ {t("common.perPerson")}</span>
            </div>
